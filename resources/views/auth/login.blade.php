<x-guest-layout>
    <div id="login-card"
         class="relative bg-white border border-red-200/70 shadow-2xl rounded-2xl p-0 overflow-hidden
                max-w-2xl mx-auto flex flex-row min-h-[430px]">
        {{-- Kiri: Brand + gambar dengan overlay --}}
        <div id="logo-section"
             class="relative flex flex-col items-center justify-center w-1/2 min-w-[220px] max-w-[260px] overflow-hidden">
            {{-- Background image --}}
            <img
            ...
            />
            {{-- Overlay gelap + blur supaya teks/ logo kebaca --}}
            <div class="absolute inset-0 bg-black/40 backdrop-blur-sm"></div>

            {{-- Isi logo + tagline di atas overlay --}}
            <div class="relative z-10 flex flex-col items-center p-6 gap-4">
                <img id="brand-logo"
                     src="{{ asset('img/logo-mygroceriesround.png') }}"
                     alt="Logo"
                     class="w-20 h-20 rounded-xl border-4 border-white bg-white shadow-xl mb-2 opacity-0 scale-110 transition-all duration-700 ease-in-out"/>
                ...
                <div id="tagline"
                    ...
                    <div class="mt-4 text-[12px] text-white/80">Silakan login untuk mulai mengatur keuangan</div>
                </div>
            </div>
        </div>

        {{-- Kanan: Login Form --}}
        <div class="flex-1 flex items-center bg-white p-8">
            <form method="POST" action="{{ route('login') }}" class="w-full space-y-5 z-10">
                ...
                <div>
                    ...
                    <x-text-input id="email"
                        class="block mt-1 w-full bg-white border border-red-200 rounded-xl text-red-900 placeholder:text-red-400 focus:ring-red-400"
                ...
                <div class="relative">
                    ...
                    <x-text-input id="password"
                        class="block mt-1 w-full bg-white border border-red-200 rounded-xl text-red-900 placeholder:text-red-400 focus:ring-red-400 pr-10"
                ...
                <div class="flex justify-center mt-2">
                    <button type="submit"
                        class="w-full md:w-auto px-10 py-2 bg-gradient-to-r from-red-600 via-red-500 to-rose-400 shadow-md rounded-xl text-white font-bold
                               hover:scale-105 hover:from-red-700 hover:to-red-600 transition-all tracking-wider border-none">
                        Log in
                    </button>
                </div>

                {{-- Daftar gratis: gaya oval gradien mirip register --}}
